...
const ChatMessages = forwardRef(
  ({ selectedUser, currentUser, setReplyMessage, search }, ref) => {
    ...
    useEffect(() => {
      const handler = (msg) => {
        ...
        setChatMessages((prev) => {
          ...
          // Replace optimistic temp entry (sender's own message echoed back).
          // Keep blob image so UI doesn't flicker while Cloudinary upload runs.
    ...
    // Server broadcasts after update-message-image saves the Cloudinary URL.
    // By now the optimistic entry has been replaced with real _id, so patch by messageId.
    useEffect(() => {
      const handler = ({ tempId, messageId, image }) => {
        const patchImage = (prev) =>
          prev.map((msg) => {
            if (String(msg._id) === String(messageId) || msg._id === tempId) {
              return { ...msg, image, status: "sent" };
            }
            return msg;
          });

        setChatMessages(patchImage);
        setSearchMessages(patchImage);
      };
